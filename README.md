...
**扫码确认了但插件没反应，最后说超时** — 抖音的二维码约每 60 秒作废一张，页面会自己换新码，但 QQ 里发出去的那张图不会更新，扫作废的码永远等不到结果。插件现在检测到换码就补发一张新图并注明「上一张已过期，请扫这张新的」，扫中后先回「已收到扫码，请在手机上点确认登录」。真的超时也会区分「压根没扫」和「扫了但没等到确认」两种文案。
