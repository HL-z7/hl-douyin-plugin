...
**二维码一直不出** — 未登录访问抖音首页时，登录面板是抖音自己弹的，插件只负责等它出现；实测从页面就绪到二维码渲染要 20 秒上下，所以 `security.qrLoginTimeout` 别调到 60 秒以下。若报「未出现登录面板」，多半是被风控挡在验证码中间页，`spark.headless` 改成 `false` 看一眼实际页面，或退回文件/手动导入 Cookie。

**扫码确认了但插件没反应，最后说超时** — 先看是哪一种。若停在「已收到扫码」：抖音的二维码约每 60 秒作废一张，页面会自己换新码，但 QQ 里发出去的那张图不会更新，扫作废的码永远等不到结果；插件现在检测到换码就补发一张新图并注明「上一张已过期」。若已经回过「已确认登录，正在取凭证」还是失败：抖音在扫码后追加了一道**身份验证**——实测截图里是一个标题为「身份验证」的弹窗，副标题「为保障账号安全，请先完成身份验证，以确保为本人操作」，下面两个选项「接收短信验证码」「发送短信验证」，二维码已被它整块盖掉。这道验证必须由人在浏览器里点，无头模式下谁也代不了。插件会认出来并立刻说明，同时在 `data/screenshots/qrlogin-*.png` 留一张现场截图。两条出路：

1. **改用 `#抖音文件登录 账号名`**（推荐）——在你自己的浏览器里登录抖音，验证一次就过，把 `document.cookie` 存成 txt 私聊发过来
2. **把 `spark.headless` 改成 `false` 再扫一次** —— 浏览器窗口会显形在跑 Yunzai 的那台机器上，你自己在里面把身份验证过掉，Cookie 照样会被插件抓走存下来。这条路要求你能看见那台机器的桌面
