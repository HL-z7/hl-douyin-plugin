...
**报 `未找到登录入口` / 页面标题是「验证码中间页」** — 无头 Chromium 的默认 UA 带 `HeadlessChrome`，抖音见到就返回一个 4KB 的验证码中间页，既没有登录入口也没有聊天列表。插件已在建页时把 UA 里的 `HeadlessChrome` 换成 `Chrome`，遇到这条说明用的是旧代码，`#抖音更新` 拉一下即可。
