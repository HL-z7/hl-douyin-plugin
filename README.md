...
**扫码确认了但插件没反应，最后说超时** — 先看是哪一种。若停在「已收到扫码」：抖音的二维码约每 60 秒作废一张，页面会自己换新码，但 QQ 里发出去的那张图不会更新，扫作废的码永远等不到结果；插件现在检测到换码就补发一张新图并注明「上一张已过期」。若已经回过「已确认登录，正在取凭证」还是失败：抖音在扫码后追加了一道验证（短信验证码 / 滑块 / 安全验证），无头浏览器里没人能替你点，插件会在认出来后立刻说明，并在 `data/screenshots/qrlogin-*.png` 留一张现场截图。这种情况扫码这条路走不通，改用 `#抖音文件登录 账号名`——在你自己的浏览器里登录（验证一次就过），把 `document.cookie` 存成 txt 发过来。

 — 无头 Chromium 的默认 UA 带 `HeadlessChrome`，抖音见到就返回一个 4KB 的验证码中间页，既没有登录入口也没有聊天列表。插件已在建页时把 UA 里的 `HeadlessChrome` 换成 `Chrome`，遇到这条说明用的是旧代码，`#抖音更新` 拉一下即可。
