...
export async function attachInterception(page, opts = {}) {
  ...
  debug("浏览器", `已挂请求拦截：资源=${blockResources ? "拦" : "放"} 埋点/推荐流=${blockTracking ? "拦" : "放"}`)
  page.on("request", req => {
    try {
...
class BrowserManager {
  ...
  async openPage(cookies = [], opts = {}) {
    ...
    try {
      ...
      debug("浏览器", `新页面就绪：cookie=${cookies?.length || 0} 条 拦截=${opts.intercept === false ? "关" : "开"} viewport=1280x800`)
      return { context, page }
    } catch (error) {
