 ...
 */
async function builtinChromium() {
  try {
    /**
     * puppeteer 22+ 的 executablePath() 返回 Promise，必须 await，
     * 否则会把 "[object Promise]" 当成浏览器路径传给 launch。
     */
    const path = typeof puppeteer.executablePath === "function" ? await puppeteer.executablePath() : ""
...
class BrowserManager {
  ...
  async #launchOptions() {
    ...
    const chromium = custom || cfg?.bot?.chromium_path || (await builtinChromium())
    const options = {
