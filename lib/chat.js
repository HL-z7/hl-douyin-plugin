...
class ChatSession {
   ...
   * 因此前端能看到历史上出现过的全部会话。
   *
   * 读列表前先把搜索框清空：openPeer 走 searchConversation 会把昵称留在框里，搜索面板一直
   * 开着并盖住会话列表，此后每次刷新都只读到搜索结果那几条（spark 自己的列表兜底
   * findInChatList 也是这么做的）。
   *
  ...
  async conversations() {
    this.#assertLive()
    // 清空搜索词并等面板收起，否则读到的是上一次搜索的结果项
    await clearSearch(this.page).catch(() => {})
    const items = await readItems(this.page, SEL.conversation, (...a) => this.trace(...a))
    const seen = []
    const dropped = []
    for (const item of items) {
      ...
      if (seen.some(s => s.peer === peer)) {
        dropped.push(`${peer}(重复)`)
        continue
      }
      seen.push({ peer, ...itemPreview(item, peer) })
    }
    // 丢弃的也报出来：条数与昵称数不一致时，「取不到名字」与「同名重复」排查方向不同
    this.trace(
      `会话列表读到 ${seen.length} 条${dropped.length ? `，丢弃 ${dropped.length} 条（${dropped.join("、")}）` : ""}`
    )
 ...
 * 2. 在根节点内按父节点归组，取成员最多的一组作为「气泡行」
 * 3. 每条气泡取整条 innerText，剔除纯时间/状态的行
 * 4. self 先向上查找 right/self/mine 一类的 class，命中不了则退到「气泡在容器内偏右」
 *
 * 第 2 条与 spark.readItems 判据不同（那边按 class token 归组）：消息气泡的容器在会话内是同
 * 一批同父兄弟，父节点归组成立；而会话列表被虚拟列表给每行套了一层定位容器，父节点两两不同，
 * 只能靠 class token。两处各按自己那份 DOM 的实际形状取判据，不强行统一。
 *
