...
/**
 * 一条消息的去重键：`self + text + stamp`。抖音不给消息 id，也没有结构化时间，
 * 只能拿「谁说的 + 说了什么 + 页面上那行时间文本」当身份。
 *
 * 分隔符用 \x1f（ASCII 单元分隔符）而不是空格：昵称和正文里都可能有空格，
 * 用可打印字符当分隔符会让不同的三元组拼出同一个键。
 */
function key(m) {
 ...
 * 的某个后缀等于本屏的某个前缀时，本屏余下的部分才是新消息。
 *
 * 这个判据对「连发两条一样的话」是保守的：它会认为第二条是重复而丢掉。宁可漏一条也不要每
 * 轮把历史翻倍，是刻意选的 —— 重复插入是不可逆的污染，漏掉的那条下次页面刷新还在。
 *
 * 空文本气泡（图片、表情、撤回占位）在这里被 filter 掉：文本是去重键的一部分，
