 ...
 * （push.target 的合法取值）、`configPath`（配置文件的绝对路径，lib/reload.js 监听它）、
 * `config` 单例。
 *
 * 依赖：只依赖 `lib/util.js` 的纯函数（deepMerge / getPath / setPath / toIdList / log /
 * toError）与 yaml 包，不导入插件内其它模块 —— 几乎所有模块都要读配置，
 * 反向依赖会形成循环。
 *
...
import { pluginRoot, ensureDir, deepMerge, getPath, setPath, log, oneLine, toError, toIdList } from "./util.js"

const configDir = ensureDir(pluginRoot, "config")

/** 配置文件的绝对路径。lib/reload.js 靠它定位要监听的文件 */
...
/**
 * 判断解析出来的 YAML 是否「像本插件的配置」。返回空串表示可用，否则返回给人看的原因。
 *
 * 只挡一类输入：顶层没有任何 DEFAULT_CONFIG 里声明过的段。它能覆盖好几种「语法合法但显然
 * 不是配置」的内容 —— 空文件（YAML.parse 给 null）、纯标量、数组，以及编辑器把文件写到一半
 * 或脚本写错后剩下的陌生键（例如 `::: not yaml` 会被 YAML 当成键 `::` 收下）。而本插件的
 * 每一项配置都挂在这几个段下面，所以这条规则挡不住任何正常写法：配置文件允许只写改过的
 * 字段，只写一段 `debug:\n  enable: true` 完全合法。
 *
 * 之所以要在「语法错误」之外再挡这一层：热重载（lib/reload.js）读到这种文件时若照收，
 * 正在生效的配置会被整套换成默认值 —— 定时任务按默认 cron 重排、推送目标清空，而用户只是
 * 保存到了一个残片。
 *
 * @param {*} doc YAML.parse 的结果
 * @returns {string} 不可用时的原因，可用时为空串
 */
function explainShape(doc) {
  if (!doc || typeof doc !== "object" || Array.isArray(doc))
    return "顶层不是一个配置对象（应该是 `spark:` / `debug:` 这样的段）"
  if (!Object.keys(doc).some(key => Object.hasOwn(DEFAULT_CONFIG, key)))
    return `顶层没有任何已知配置段（可用：${Object.keys(DEFAULT_CONFIG).join(" / ")}）`
  return ""
}
...
class Config {
  ...
  constructor() {
    ...
    if (error) log("error", "config.yaml 不可用，本次使用默认配置：", error.message)
    this.#data = data
  }

  /**
   * 读盘并与默认值深合并。不认识「读没读到」以外的任何事：是否采用这份结果由调用方决定，
   * 因为「加载期读到坏文件」与「热重载时读到坏文件」要采取相反的动作（前者退回默认值，
   * 后者保留现有配置）。
   *
   * 除了 YAML 语法错误，还挡一类「能解析但明显不是本插件配置」的文件（见 explainShape）：
   * 编辑器把文件写到一半时留下的残片完全可能是合法 YAML，若照收，热重载会把正在生效的
   * 配置整套换成默认值。
   *
   * @returns {{data: object, error: Error|null, missing: boolean}} 字段不存在时 missing
   *   为 true，且 error 为 null —— 全新安装没有 config.yaml 是正常状态，不是故障
   */
  #read() {
    if (!fs.existsSync(configPath)) return { data: deepMerge(DEFAULT_CONFIG, {}), error: null, missing: true }
    let user = {}
    try {
      user = YAML.parse(fs.readFileSync(configPath, "utf8"))
    } catch (error) {
      return { data: deepMerge(DEFAULT_CONFIG, {}), error: toError(error), missing: false }
    }
    const bad = explainShape(user)
    if (bad) return { data: deepMerge(DEFAULT_CONFIG, {}), error: new Error(bad), missing: false }
    return { data: deepMerge(DEFAULT_CONFIG, user || {}), error: null, missing: false }
  }

  /**
   * 重新读盘。外部（锅巴 / Web 面板 / `#抖音设置` / 手改文件）改动后调用，使进程内立即生效。
   *
   * @param {{strict?: boolean}} [opts] strict=true 时不接受「读不出来」这个结果：文件不存在、
   *   YAML 语法错误、结构不像本插件配置都返回 null 且**不改动内存里的配置**，由
   *   lib/reload.js（配置热重载）使用。理由是那时配置已经生效并在被使用，把一份半写坏的 yaml
   *   读成默认值等于悄悄把这一堆开关全拨到默认档（定时任务会被注销、推送目标会清空），
   *   比「这次改动没生效」严重得多。默认 false 保持原语义：退回默认配置树。
   * @returns {object|null} 新的配置树；strict 模式下读盘失败时返回 null
   */
  reload({ strict = false } = {}) {
    const { data, error, missing } = this.#read()
    if (strict && (error || missing)) {
      // yaml 的报错自带多行定位图（行号 + 插入符），压成一行再进日志：混在别的日志里也能读
      const why = missing ? "不存在" : `读不出来（${oneLine(error.message, 160)}）`
      log("warn", `config.yaml ${why}，本次热重载作废，继续用当前配置`)
      return null
    }
    if (error) log("error", "config.yaml 不可用，本次使用默认配置：", error.message)
    this.#data = data
