 ...
 */
export async function noteField(page, x, y) {
  try {
    return await page.evaluate(
      ([px, py, selector]) => {
        const hit = document.elementFromPoint(px, py)
...
export async function clickAt(page, x, y, opts = {}) {
   ...
   */
  const arrived = Boolean(probe && probe.events.length)
  let forced = ""
  if (probe && (!arrived || probe.mutations === 0)) {
    forced = await forceClick(page, x, y)
    debug(scope, `  ${arrived ? "事件到了但组件没动" : "真鼠标没进 DOM"}，已补合成事件：${forced}`)
  }
   ...
   */
  const field = opts.focusField === false ? "" : await noteField(page, x, y)
...
/**
 * 元素当前在视口里的位置与尺寸，顺手把它滚进视口。
 *
 * 不用 `handle.boundingBox()` 了事：它对 `display:none` 返回 null，但对「在滚动容器
 * 外面」的元素返回的是视口外坐标，点上去会落在别的元素身上。所以自己读一遍 rect，
 * 并把「在不在视口内」一起判掉。
 *
 * @returns {Promise<{x: number, y: number, cx: number, cy: number, w: number, h: number} | null>}
 */
export async function visibleBox(handle) {
  ...
  try {
    return await handle.evaluate(el => {
      ...
      return { x: r.left, y: r.top, cx, cy, w: r.width, h: r.height }
    })
  } catch {
    return null
  }
...
export async function clickHandle(page, handle, opts = {}) {
  ...
  const r = await clickAt(page, box.cx, box.cy, opts)
...
export async function clickByText(page, re, opts = {}) {
  ...
  const found = await page
    .evaluate(
      (source, flags, sel, scopeEl) => {
        ...
        const hits = [...base.querySelectorAll("*")].filter(el =>
          test.test((el.textContent || "").replace(/\s+/g, " ").trim())
        )
        // 只留最内层：有后代也匹配的说明它只是个包装，点它会落在整块面板的几何中心
        const leaves = hits.filter(el => !hits.some(other => other !== el && el.contains(other)))
        const pick = leaves.find(el => {
          const r = el.getBoundingClientRect()
          return r.width >= 1 && r.height >= 1
        })
        if (!pick) return { found: false, total: hits.length, text: "" }
        pick.setAttribute(sel, "1")
        return { found: true, total: hits.length, text: (pick.textContent || "").replace(/\s+/g, " ").trim().slice(0, 24) }
      },
      re.source,
      re.flags,
      attr,
  ...
  try {
    ...
    debug(scope, `点「${found.text}」：${r.ok ? `事件${r.arrived ? "已到 DOM" : "⚠ 未到 DOM"}${r.forced ? `，已补合成` : ""}` : r.reason}`)
