 ...
 * （基础能力与诊断），clickAt / typeText / clearText / pressKey（坐标级动作），visibleBox /
 * clickHandle / clickByText（元素级封装）。
 *
 * 输入有三条通道，依次退化（见 typeText）：真键盘 `Input.dispatchKeyEvent` → 可信文本插入
 * `Input.insertText` → 合成 InputEvent。第二条是现场加的：抖音验证码框在无头环境下前者进不了
 * DOM、后者被 React 受控组件的下一轮渲染覆盖掉，只有它两头都能过。
 *
 ...
 * @returns {Promise<{ok: boolean, note: string}>} ok 表示写完当场回读到的内容与 text 一致
 *   （只证明这一次写入落到了 DOM 上，不代表组件认账，那要等下一轮渲染才知道）；
 *   note 是供日志使用的一句结论。找不到输入框或 evaluate 抛错时 ok 为 false，不抛出
 */
  ...
  try {
    return await page.evaluate(
      ([str, selector]) => {
        ...
        if (!field || !field.isConnected) return { ok: false, note: "找不到输入框（先在画面上点一下输入框）" }
        try {
          field.focus({ preventScroll: true })
        } catch {}
        ...
        const tag = field.tagName.toLowerCase() + (field.id ? `#${field.id}` : "")
        // 当场回读：React 受控组件会在收到 input 后用自己的 state 覆盖 value，
        // 若它不认这一轮合成事件，这里读到的就不是刚写进去的内容
        const now = String((editable ? field.textContent : field.value) ?? "")
        return {
          ok: now === str,
          note: `已往 ${tag} 补写 ${str.length} 字${now === str ? "" : `，但回读只有 ${now.length} 字`}`,
        }
      },
      [String(text), FIELD_SELECTOR]
    )
  } catch (error) {
    return { ok: false, note: `补写失败：${error.message}` }
  }
}

/**
 * 输入前重申焦点：确认它还在上次点中的那个输入框上，不在就设回去。
 *
 * 三条输入通道都没有坐标 —— 真键盘与 insertText 都只往 `document.activeElement` 送字，
 * 合成输入也是先看 activeElement 再退回 `window.__dyField`。因此「焦点在不在输入框里」是
 * 这三条路共同的前置条件，而它在点击之后并不保证仍然成立：抖音那个验证面板是 React 的，
 * 点击引发的重渲染可能把焦点弹回 body，倒计时（`44s后重新发送`）每秒一次的更新也可能
 * 顺带重建节点。上一版只在 clickAt 末尾的 noteField 里确认过一次，之后就不再管。
 *
 * 不重新点一次而是直接 focus()：`el.focus()` 是页面自身的 API，不经过输入事件通道，
 * 而现场那条链路真鼠标事件根本进不了 DOM（探针报「到达 DOM 的事件 ⚠ 一个都没有」）——
 * 再点一次只会重复一次已知失败的动作。
 *
 * @param {import("puppeteer").Page} page
 * @returns {Promise<{name: string, refocused: boolean}>} name 是最终持有焦点的输入框
 *   `tag#id`，一个都找不到时为空串；refocused 表示是否真的动手设回过焦点
 */
async function ensureField(page) {
  try {
    return await page.evaluate(selector => {
      const active = document.activeElement
      if (active?.matches?.(selector)) {
        const cls = active.tagName.toLowerCase() + (active.id ? `#${active.id}` : "")
        return { name: cls, refocused: false }
      }
      const field = window.__dyField
      if (!field?.isConnected) return { name: "", refocused: false }
      try {
        // preventScroll：验证弹窗是 position: fixed，聚焦引发滚动会改变后续点击的坐标基准
        field.focus({ preventScroll: true })
      } catch {
        field.focus()
      }
      const tag = field.tagName.toLowerCase() + (field.id ? `#${field.id}` : "")
      return { name: document.activeElement === field ? tag : "", refocused: true }
    }, FIELD_SELECTOR)
  } catch {
    return { name: "", refocused: false }
  }
}

/**
 * 兜底：走 `Input.insertText` 逐字符插入。
 *
 * 这一层夹在真键盘与合成事件之间，补的是两者中间那道缝：
 *
 * - `page.keyboard.type` 走 `Input.dispatchKeyEvent`，模拟的是**按键**。它要经过完整的键盘
 *   输入管线（keydown → 编辑命令 → 文本插入），链路上任何一环不通，输入框就是空的。
 * - `forceType` 用 `dispatchEvent` 派发合成事件，`isTrusted` 为 false，值是本侧直接写进
 *   DOM 的。React 受控组件在下一轮渲染里可能拿自己的 state 覆盖回去，于是「写进去了、又被
 *   清掉了」——现场表现就是提交时输入框仍为空。
 * - `Input.insertText` 走的是 Blink 的文本插入管线（等同于输入法提交一段文本）：由浏览器
 *   自己派发 beforeinput/input，`isTrusted` 为 true，React 的 onChange 照常触发，且不依赖
 *   keydown 那条链路通畅。
 *
 * 用 `keyboard.sendCharacter` 而不自建 CDP 会话：它的实现就是
 * `client.send("Input.insertText", { text: char })`（puppeteer-core 23.10.1
 * cdp/Input.js:122-124），走的是页面已有的那条会话。lib/spark.js:1226 发抖音私信用的也是它。
 *
 * 逐字符而非整串一次插入：抖音的验证码框每满一位判定一次（满 6 位自动提交），整串插入时它
 * 只看到最后一次变化。
 *
 * 前置条件是焦点必须已在输入框上（insertText 没有坐标，插到 activeElement）。调用方在
 * typeText 里已由 noteField 保证过这一点。
 *
 * @param {import("puppeteer").Page} page
 * @param {string} text 要插入的内容
 * @returns {Promise<{ok: boolean, note: string}>} ok 表示插完回读到的内容长度与 text 一致；
 *   老内核没有该 CDP 命令、或插入过程抛错时 ok 为 false，不抛出
 */
async function insertText(page, text) {
  try {
    for (const ch of text) {
      await page.keyboard.sendCharacter(ch)
      // 与真键盘的 delay 同量级：抖音的验证码框监听输入频率，插太快像脚本
      await sleep(40)
    }
    const now = await page.evaluate(selector => {
      const el = document.activeElement
      const field = el?.matches?.(selector) ? el : window.__dyField
      if (!field?.isConnected) return -1
      return String((field.isContentEditable ? field.textContent : field.value) ?? "").length
    }, FIELD_SELECTOR)
    return {
      ok: now === text.length,
      note: `insertText 插了 ${text.length} 字${now === text.length ? "" : `，但回读只有 ${now < 0 ? "读不到" : now} 字`}`,
    }
  } catch (error) {
    return { ok: false, note: `insertText 失败：${error.message}` }
  }
}
...
/**
 * 往当前焦点元素输入文本，真键盘写不进去则依次退到 insertText 与合成输入。
 *
 * @param {import("puppeteer").Page} page
 ...
 * @returns {Promise<{grew: boolean, forced: string, focus: string, ok: boolean}>} grew 表示真
 *   键盘是否让内容长度发生变化；forced 为空串表示未走兜底；focus 是输入前的焦点元素名；
 *   ok 表示最终输入框里确实有内容（三条路任一成功即为 true）
 */
export async function typeText(page, text, opts = {}) {
  const scope = opts.scope || "页面交互"
  const str = String(text ?? "")
  if (!str) return { grew: false, forced: "", focus: "", ok: false }

  /*
   * 先确认焦点还在输入框里。
   *
   * 三条输入通道都只往 activeElement 送字，焦点丢了就等于往页面空处打字 —— 现场表现正是
   * 「日志说填了 6 位、截图里输入框仍是灰色 placeholder」。点击那一步（clickAt 末尾的
   * noteField）确实设过焦点，但那之后隔着一次 React 重渲染与倒计时刷新，到这里未必还在。
   */
  const held = await ensureField(page)
  if (held.refocused) debug(scope, `  输入前焦点已不在输入框，已设回 ${held.name || "⚠ 失败"}`)
  else if (!held.name) debug(scope, `  ⚠ 输入前找不到任何输入框（没点过，或它已被重建）`)
   ...
   *
   * 两级兜底的顺序不能反。insertText 由浏览器自己派发 input 事件（isTrusted 为 true），React
   * 受控组件会正常走 onChange 并把值收进 state；而合成事件的值是本侧直接写进 DOM 的，组件下
   * 一轮渲染可能拿旧 state 覆盖回去 —— 现场那次「已补写 6 字」之后提交仍报没拿到凭证，就是
   * 这一种。所以先试可信通道，只有它也不成才落到合成事件。
   */
  const grew = Boolean(before && after >= 0 && after !== before.length)
  let forced = ""
  let ok = grew
  if (!grew) {
    const ins = await insertText(page, str)
    forced = ins.note
    ok = ins.ok
    debug(scope, `  真键盘没让内容变化（焦点=${before?.name || "?"}），改走 insertText：${ins.note}`)
    if (!ins.ok) {
      const syn = await forceType(page, str)
      forced = `${ins.note}；${syn.note}`
      ok = syn.ok
      debug(scope, `  insertText 也没写进去，再补合成输入：${syn.note}`)
    }
  }
  return { grew, forced, focus: before?.name || "", ok }
}

/**
 * 清空一个输入框，真键盘删不掉则改写值并补输入事件。
 *
 * 单开一个入口而不复用 pressKey("Backspace")：后者的合成兜底「一律删除末位字符」（光标位置
 * 在合成事件里不可靠，见 forceKey），对验证码框够用，但清空搜索框要删的是整串 —— 实测在
 * keydown 被拦的页面上，`select()` + Backspace 只删掉一个字符，读回是 `ciall`。
 *
 * 两级：先按一次 Backspace（选中全部的情况下真键盘会整段删掉），回读仍非空再直接改值。
 * 改值那一级与 forceType 同一套写法：用原型上的 value setter 绕过 React 的属性劫持，
 * 并派发 `deleteContentBackward` 的 InputEvent 让受控组件收到 onChange。
 *
 * @param {import("puppeteer").Page} page
 * @param {import("puppeteer").ElementHandle} [handle] 要清空的元素。不传则作用于
 *   `document.activeElement`（退回 `window.__dyField`），与三条输入通道一致
 * @param {object} [opts]
 * @param {string} [opts.scope] debug 日志的定位串
 * @returns {Promise<{ok: boolean, note: string}>} ok 表示清完回读为空；note 供日志使用
 */
export async function clearText(page, handle, opts = {}) {
  const scope = opts.scope || "页面交互"
  // 选中全部：select() 是页面自身的 API，不经输入事件通道，比三连击可靠
  if (handle) await handle.evaluate(el => el.select?.()).catch(() => {})
  await pressKey(page, "Backspace", { scope })
  ...
  const left = await readLen()
  if (left <= 0) return { ok: left === 0, note: left === 0 ? "Backspace 已清空" : "清空后读不到输入框" }

  const forced = await page
    .evaluate(selector => {
      const active = document.activeElement
      const field = active?.matches?.(selector) ? active : window.__dyField
      if (!field?.isConnected) return "找不到输入框"
      const editable = field.isContentEditable
      const proto = field instanceof HTMLTextAreaElement ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype
      const setter = Object.getOwnPropertyDescriptor(proto, "value")?.set
      field.dispatchEvent(
        new InputEvent("beforeinput", { bubbles: true, cancelable: true, composed: true, inputType: "deleteContentBackward" })
      )
      if (editable) field.textContent = ""
      else if (setter) setter.call(field, "")
      else field.value = ""
      field.dispatchEvent(
        new InputEvent("input", { bubbles: true, composed: true, inputType: "deleteContentBackward" })
      )
      field.dispatchEvent(new Event("change", { bubbles: true }))
      return `已把 ${field.tagName.toLowerCase()} 的值改成空串`
    }, FIELD_SELECTOR)
    .catch(error => `改值失败：${error.message}`)

  const after = await readLen()
  const note = `Backspace 后还剩 ${left} 字，${forced}${after === 0 ? "" : `，仍剩 ${after < 0 ? "读不到" : after} 字`}`
  debug(scope, `  ${note}`)
  return { ok: after === 0, note }
}
