...
/**
 * 终止一个登录会话。why 会成为会话的最终文案，前端与指令轮询都读它，
 * 所以「用户点了取消」和「进程要退出了」得说得出区别。
 */
...
/**
 * 终止所有还在进行的扫码会话，进程退出/重启前调用。
 *
 * 与 cancelUserSessions 的区别只在「筛不筛发起人」，但必须单独有一个：退出时没有
 * 「当前用户」这个概念，而进行中的会话每一个都占着一个 page 和一个 context
 * （见 cleanup），不收就跟着 Chromium 一起留在系统里。
 *
 * 走 canceled 这条终态而不是直接删表：cleanup 会 clearTimeout 摘掉超时兜底，
 * 而超时与失败两处兜底都先看状态，所以半途的那一路不会在退出过程中再往外推消息。
 *
 * @returns {Promise<number>} 真的被取消掉的会话数
 */
 ...
 *
 * 文本只从验证面板里取，不读整页 body。那个面板是浮在抖音首页上的一层
 * （现场是 `div#uc-second-verify`），而 `document.body.innerText` 会把底下推荐流的
 * 一切也带上：几十个视频标题、作者名、时长。后果有两条，都实打实踩过 ——
 *   - 判 `sent` 的 `\d+s后` 会被视频时长撞上，于是跳过「发送验证码」不点，
 *     用户手上压根没有短信却被要求提供验证码；
 *   - 报错时截的那段文本全是首页内容，弹窗上写的什么一个字都看不到（现场
 *     `describeLine` 报的是 `/jingxuan` 的文本，排查时完全没用）。
 * 面板认不出来时才退回整页，那时读数偏吵，但总比什么都读不到强。
 *
 * @returns {Promise<{text: string, scope: string, entry: boolean, sendBtn: boolean, sent: boolean, fields: number, phone: string} | null>}
 */
async function readSmsState(page) {
  ...
  return page
    .evaluate(hints => {
      /*
       * 找验证面板：从「写着验证相关字样的最内层元素」往上找带 verify/captcha/dialog
       * 这类标识的外壳。不直接 querySelector 那些 class —— 抖音的 class 是混淆的，
       * 而 id/class 里的 `uc-second-verify`、`dialog` 这类语义词是框架给的，稳得多。
       */
      const label = el => {
      ...
      const panel = (() => {
        ...
        /*
         * 往上爬，取**最外层**那个命中的祖先，不是最里层的。
         *
         * 这里以前是「命中就 return」，现场因此栽了一次：抖音把
         * `uc_verification_component_*` 这种 class 挂在面板里每个小组件上，而 hints
         * 里有 `verification` —— 于是「写着『接收短信验证码』的那个 typography div」
         * 自己就命中了，panel 就是它，读出来的文本只有那五个字。后果是第二屏明明
         * 已经写着「短信已发送至 190******31 / 45s后重新发送」，状态机却一路读到
         * sent=false，白白改走远程验证页面 —— 用户手机上其实早就收到短信了。
         * interact.js 的 watchClick 取的就是最外层（它报的 scope 是
         * `div#uc-second-verify`，那才是整个面板），两处保持一致。
         *
         * 唯一要防的是爬太外把底下的推荐流也圈进来，所以带一个文本长度上限：
         * 这个弹窗满打满算百来字，而首页推荐流的 innerText 是几十 KB 起。
         */
        let found = null
      ...
      // 用户手上压根没有短信却被要求提供验证码。全插件只有这一处判 sent，waitSmsSent 也复用它
 ...
 * 轮询等「短信真的发出去了」。
 *
 * 判据复用 readSmsState 的 `sent`，不在这里另写一条正则 —— 上一版就是两处各写一份
 * 正则、又各自读整页 `document.body.innerText`，结果同一个页面能读出两个结论：
 * 首页推荐流里一个 "15s" 就能让这里判「已发送」而状态机判「没发送」。
 * 现在两者同源，且都只读验证面板内的文本。
 *
 * @param {number} [timeout=8000] 总等待毫秒；抖音这一步是「点完 → 服务端受理 → 倒计时起」，
 *   慢的时候要三四秒
 * @returns {Promise<object|null>} 已发送时返回那一刻的 state，超时返回 null
 */
...
async function tryAutoSms(s, setStatus, hint, ttl) {
  ...
  /*
   * ① 「接收短信验证码」是那个弹窗第一屏上的一个选项，点它之后才会切到第二屏
   *    （手机号 + 「获取验证码」+ 验证码输入框）。
   *
   * 判据只看 entry，不再看 sendBtn —— 现场就是被 sendBtn 坑掉的：抖音那个面板把两屏
   * 的 DOM 同时渲染出来，第二屏压在第一屏底下，`document.body.innerText` 里于是同时
   * 有「接收短信验证码」和「获取验证码」。上一版的条件是 `entry && !sent && !sendBtn`，
   * sendBtn 为 true 就跳过了这一步，直接去点第二屏那个被压住的「获取验证码」——
   * 真鼠标点在压在上面的选项列表上（探针报「事件一个都没有」），而抖音压根没收到
   * 「我要用短信验证」这个选择，8 秒后自然等不到「已发送」。
   *
   * 点不动时不立刻放弃：sendBtn 已经在页面上（说明可能本来就在第二屏），
   * 让它往下走一步试试，比直接退回远程验证页面强。
   */
  if (state.entry && !state.sent) {
    ...
    if (!r.ok) debug(tag, `自动短信：点不到入口（${r.reason}），但页面上已有发送按钮，接着往下试`)
       ...
       * 等第一屏真的退场，再去点第二屏的按钮。
       *
       * 固定 sleep 在这里不够：两屏叠着渲染时，「获取验证码」从一开始就在 DOM 里，
       * 睡够了也判不出切没切。真正的判据是「它现在是不是最上面那个」——
       * elementFromPoint 认到它（或它的后代）才说明第一屏那层列表让开了。
       */
  ...
  if (!state.sent) {
    ...
    if (!send.ok) {
      // 报的是面板里的文本，不是 describeLine 那一整页 —— 出问题时要看的是弹窗上写了什么，
      // 而整页文本会被底下推荐流的视频标题灌满（现场报的是 /jingxuan 的内容，毫无用处）
...
export async function submitSmsCode(sessionId, code) {
  ...
  log("warn", `${tag}：验证码已提交但还没拿到凭证。面板(${state?.scope || "读不到"})：${state?.text || ""}`)
  return {
    ok: false,
