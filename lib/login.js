...
async function runQrLogin(s, setStatus, onQrcode) {
  ...
  //
  // 二维码有两个来源：上面 response 钩子拦到的 get_qrcode 响应（首选，还带 status 字段），
  // 以及页面上真实渲染出来的那张 base64 图。哪个先到就用哪个，所以这里轮询而不是
  // 死等 DOM —— 响应先到时立刻收工，不必再陪 DOM 等到超时。
  //
  // 实测面板弹出后二维码还要异步请求，首页从 DOM 就绪到出码约 20 秒，所以上限给 45 秒，
  ...
  while (!qrSent && Date.now() < qrDeadline) {
    if (["failed", "expired", "canceled"].includes(s.status)) break
    if (!s.page || s.page.isClosed()) break
    const domQr = await findQr()
    if (domQr) {
      emitQr(domQr)
      break
    }
