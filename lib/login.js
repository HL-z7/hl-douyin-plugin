...
export async function startQrLogin(botId, { accountName = "", onQrcode, onStatus } = {}) {
  ...
  s.timer = setTimeout(async () => {
    ...
    if (s.status === "verify")
      message = `抖音要求补充验证（短信验证码 / 滑块等），网页端已挡在验证页，${waited} 内未通过。请改用「#抖音文件登录 账号名」导入 Cookie`
    else if (s.status === "confirmed")
      message = `已在手机上确认，但 ${waited} 内没能取到登录凭证——多半是抖音追加了验证步骤。请改用「#抖音文件登录 账号名」导入 Cookie`
...
async function runQrLogin(s, setStatus, onQrcode) {
  ...
  while (!isDone(s)) {
    ...
    if (s.status === "confirmed" && s.confirmedAt && Date.now() - s.confirmedAt > CONFIRM_GRACE_MS) {
      ...
      setStatus(
        "failed",
        "已在手机上确认，但网页端始终没拿到登录凭证。常见原因是抖音追加了短信验证或滑块，" +
...
function markVerify(s, setStatus, hint) {
  ...
  setStatus(
    "verify",
    `抖音要求补充验证（${hint}）。网页端在无头浏览器里，插件无法代你完成，` +
      "请改用「#抖音文件登录 账号名」导入 Cookie（在你自己的浏览器里登录，验证一次就过）"
  )
}
