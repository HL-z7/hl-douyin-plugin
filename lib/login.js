...
export async function startQrLogin(
  ...
  const s = {
    ...
    verifying: false,
    /** handleSuccess 是否已进入。两个 cookie 轮询会同时命中，不锁会落盘两个账号 */
    saving: false,
    accountId: "",
    createdAt: Date.now(),
...
/** 选中的验证码框上打的临时标记。填完之后还要按它回读一次，故不在 focusCodeField 里摘掉 */
const CODE_MARK = "data-dy-code"
 ...
 *
 * 光有分数不够，还要看这个框在屏幕上点不点得到。抖音把登录面板与验证面板一起渲染，两者各有
 * 一个 `placeholder=请输入验证码 maxlength=6` 的 input，分数完全相同；而登录面板那个被
 * `div.second_verify_mask` 整块盖住，用户看不见。现场因此挑中了藏着的那个：聚焦成功、
 * insertText 回读 6 位也成功，抖音却什么都没收到 —— 三条日志全报成功，验证码却发了两遍都没
 * 过。所以排序把「点得到」放在分数之前，判据与 interact.js 的 visibleBox 一致。
 *
 * 一个特征都不命中时，只在「整页仅有一个可编辑输入框」的情况下才点它：宁可放弃，
 * 也不能把验证码打进搜索框。
 *
 * @returns {Promise<string>} 命中的输入框描述（如 `请输入验证码(maxlength=6)`），
 *   用于日志；空串表示没找到或点击失败。选中的元素上留着 CODE_MARK 标记，供
 *   readMarkedField 回读
 */
async function focusCodeField(page) {
  ...
  const picked = await page
    .evaluate(
      ([sel, hints]) => {
        ...
        /*
         * 「这个框现在点得到吗」——判据与 interact.js 的 visibleBox 同源：在框内取几个点做
         * elementFromPoint，最上层是它自己 / 它的后代 / 它的祖先，都算点得到。被别的子树盖住时
         * 一个点也命中不了，那正是登录面板里那个隐藏框的特征。
         */
        const reachable = el => {
          const r = el.getBoundingClientRect()
        ...
        // 祖先链上有 verify/dialog 一类标识 = 在验证面板内。登录面板那个隐藏框爬到 html 都没有
        const inPanel = el => {
          for (let node = el; node && node !== document.body; node = node.parentElement) {
            const key = `${node.id || ""} ${node.getAttribute?.("class") || ""}`.toLowerCase()
            if (hints.some(h => key.includes(h))) return name(node)
          }
          return ""
        }

        const cands = list.map(el => {
          const r = el.getBoundingClientRect()
          return {
            el,
            n: score(el),
            ...
            panel: inPanel(el),
            desc:
              `${name(el)} 分=${score(el)} 位置=${Math.round(r.left)},${Math.round(r.top)}` +
              ` ${Math.round(r.width)}x${Math.round(r.height)}` +
              ` 点得到=${reachable(el) ? "是" : "否"} 面板=${inPanel(el) || "⚠ 不在验证面板内"}` +
              ` placeholder=${JSON.stringify(el.placeholder || "")}`,
          }
        })
        /*
         * 排序键的顺序即优先级：先要「在验证面板内」，再要「点得到」，最后才比分数。
         * 分数相同的两个框正是现场那一对，只有前两项能把它们分开。
         */
        const ranked = [...cands].sort(
          (a, b) =>
            Number(Boolean(b.panel)) - Number(Boolean(a.panel)) ||
            Number(b.reach) - Number(a.reach) ||
            b.n - a.n
        )
        const best = ranked[0]
        const pick = best?.n > 0 ? best.el : list.length === 1 ? list[0] : null
        if (pick) pick.setAttribute(sel, "1")
        return {
          // 全部候选都报出来：同分的两个框长什么样、差在哪，只有这一份读数能看出来
          cands: cands.map(c => c.desc),
          chosen: pick ? name(pick) : "",
          found: pick
            ? `${pick.placeholder || pick.name || pick.type || "input"}${
  ...
  const cands = picked?.cands || []
  debug(
    "自动短信",
    `验证码框候选 ${cands.length} 个${picked?.chosen ? `，选中 ${picked.chosen}` : "，一个都没选中"}` +
      cands.map(c => `\n              · ${c}`).join("")
  )
  ...
  try {
    ...
    return r.ok ? picked.found : ""
  } finally {
    // 标记留在页面上，等 readMarkedField 回读完再摘（见 submitSmsCode）
    await handle.dispose().catch(() => {})
  }
}

/**
 * 回读「屏幕上那个验证码框现在有几个字」，并摘掉 CODE_MARK 标记。
 *
 * 两个读数一起给，因为单看任一个都会误判：
 *
 * - `marked` 按标记回读，对应 focusCodeField 挑中的那个节点。typeText 自己的回读看的是
 *   `document.activeElement`（或它记的 `window.__dyField`），证明的只是「字进了当时聚焦的
 *   元素」—— 现场那次它报回读 6 位，而屏幕上的框始终是空的，因为聚焦的是登录面板里那个
 *   隐藏的同名框。所以提交前必须按标记确认。
 * - `panelMax` 是验证面板内所有可见输入框里内容最长的那个的字数。React 重建节点时属性
 *   标记会随旧节点一起消失（marked 变 -1），但字可能已经进了新节点 —— 只看 marked 会把
 *   这种情况误判成「填不进去」，白开一个远程页面。
 *
 * 面板识别复用 PANEL_HINTS，与 readSmsState / interact.js 的探针同源，因此 panelMax 不会把
 * 登录面板那个隐藏框算进来。
 *
 * @param {import("puppeteer").Page} page
 * @returns {Promise<{marked: number, panelMax: number}>} marked 为标记节点的字数，标记已不在
 *   时为 -1；panelMax 为面板内最长内容的字数，面板认不出来或一个输入框都没有时为 -1。
 *   evaluate 失败时两者都是 -1
 */
async function readMarkedField(page) {
  return page
    .evaluate(
      ([sel, hints]) => {
        const len = el => String(el?.value ?? el?.textContent ?? "").length

        const marker = document.querySelector(`[${sel}]`)
        const marked = marker ? len(marker) : -1
        if (marker) marker.removeAttribute(sel)

        // 从「写着验证字样的最内层元素」往上找面板外壳，取法与 readSmsState 一致
        const marks = [...document.querySelectorAll("*")].filter(el =>
          /接收短信验证码|发送验证码|获取验证码|身份验证/.test((el.textContent || "").replace(/\s+/g, " "))
        )
        const leaf = marks.filter(el => !marks.some(o => o !== el && el.contains(o))).pop()
        ...
        if (!panel) return { marked, panelMax: -1 }

        const lens = [...panel.querySelectorAll("input")]
          .filter(el => {
            if (el.offsetParent === null || el.disabled || el.readOnly) return false
            if (["hidden", "checkbox", "radio", "submit", "button"].includes(el.type)) return false
            const r = el.getBoundingClientRect()
          ...
          .map(len)
        return { marked, panelMax: lens.length ? Math.max(...lens) : -1 }
      },
      [CODE_MARK, PANEL_HINTS]
    )
    .catch(() => ({ marked: -1, panelMax: -1 }))
}
...
export async function submitSmsCode(sessionId, code) {
  ...
  /*
   * 判「填进去了」只认验证面板内的读数，不认 typed.ok 也不认标记节点。
   *
   * 三个读数的可信度不同：
   * - `typed.ok` 看的是「当时聚焦的那个元素」。现场那次它报回读 6 位，屏幕上却是空的 ——
   *   聚焦的是登录面板里那个隐藏的同名框。
   * - `back.marked` 看 focusCodeField 挑中的节点。挑对时可信，挑错时会跟着一起误报成功，
   *   与 typed.ok 同病。因此它只进日志，不参与判定。
   * - `back.panelMax` 是验证面板内可见输入框里最长的内容。面板由 PANEL_HINTS 识别，与
   *   readSmsState 同源，隐藏框不在其中 —— 这是唯一能回答「屏幕上那个框有没有字」的读数。
   *
   * 面板认不出来时（panelMax 为 -1）才退回 marked：此时没有更好的读数，而抖音改版导致面板
   * 识别失效不该让整条路直接失败。
   */
  const back = await readMarkedField(page)
  const filled = back.panelMax >= 0 ? back.panelMax === digits.length : back.marked === digits.length
  debug(
    tag,
    `填入验证码：真键盘${typed.grew ? "生效" : `无效，${typed.forced}`}` +
      `｜面板内最长 ${back.panelMax < 0 ? "读不到（面板没认出来）" : `${back.panelMax} 字`}` +
      `｜选中的框 ${back.marked < 0 ? "标记已丢" : `${back.marked} 字`}` +
      `｜判定${filled ? "已填入" : "⚠ 没填进去"}`
  )

  /*
   * 字没进屏幕上那个框 —— 不提交，直接把页面交给人。
   *
   * 提交一个空输入框只会得到「验证码错误」，而真正的问题是这个页面不接收本侧的输入，或者填的
   * 根本不是屏幕上那个框。上一版在这里照样往下走，于是用户收到「已填进去了，但还没拿到凭证，
   * 可以把验证码再发一次」—— 再发一次会走同一条已证明不通的路，短信却在几分钟后失效。现场那次
   * 用户连发两遍，两遍都是这个结果。
   *
   * 但「回读不足」有另一种成因，必须先排掉：验证码框满 6 位时抖音可能自行提交并清空输入框，
   * 那时回读同样为 0 —— 直接判死会在真的成功之后给用户开一个没用的远程页面。因此先等一下
  ...
  if (!filled) {
    await sleep(1500)
    if (await hasLoginCookie(s.context)) {
      debug(tag, `回读不到内容，但 cookie 已到手 —— 满位自动提交那一种`)
    ...
    if (!submitted) {
      const why =
        back.panelMax >= 0
          ? `面板里那个框只有 ${back.panelMax} 字`
          : back.marked < 0
            ? "标记的节点已被重建且面板认不出来"
            : `选中的框只有 ${back.marked} 字`
      log("warn", `${tag}：验证码没进屏幕上那个框（${why}；${typed.forced || "真键盘生效但框不对"}），改走远程验证页面`)
      await snapshot(page, `09-sms-x-notype-${s.id}`)
      const opened = await markVerify(s, s.setStatus, "短信验证码（填不进屏幕上那个框）", { escalate: true })
      return {
        ok: false,
        message: opened
          ? "验证码没能填进抖音那个输入框（页面上有两个同样的框，我填的不是屏幕上那个），已给你开一个远程操作页面，链接私信发你了：在里面自己填这串验证码就行，短信还没失效。"
          : "验证码没能填进抖音那个输入框，而远程操作页面也开不出来。请改用「#抖音文件登录 账号名」导入 Cookie。",
      }
    }
    debug(tag, `回读不到内容，但面板已离开输入那一屏，按已提交继续等`)
  }
 ...
 *
 * 由 `s.saving` 保证只真正执行一次，理由见函数内的说明 —— 简言之，判 status 不够，因为它在
 * 末尾才置上，而两个 cookie 轮询会在这之前双双进来。
 */
async function handleSuccess(s, setStatus) {
  /*
   * 幂等闸门，必须是同步置位的第一件事。
   *
   * 判 `s.status === "success"` 挡不住重复进入：那个状态在本函数末尾才置上（setStatus 之前
   * 还隔着 exportCookies 与 readNickname 两个 await，后者要 page.goto 一趟，正常也要 1~5
   * 秒）。而两个 cookie 轮询同时活着 —— 主循环每 2 秒查一次，submitSmsCode 的等待循环每
   * 1.5 秒查一次 —— 于是「已进本函数但 status 还不是 success」这段窗口足够让第二次调用整个
   * 穿过判据，两次各自 upsert 一遍。
   *
   * 后果是账号列表里同一个号出现两条：先跑完的那次写入真实昵称，后一次的 readNickname 撞上
   * 已被 cleanup 置空的 s.page，昵称取空退到 `抖音账号${随机四位}`，而 upsert 按名字判重，
   * 名字不同即插新记录。
   */
  if (s.saving || s.status === "success") return
  s.saving = true
  ...
  // uid 交给 store 当判重键，优先于名字：昵称会变、读不到时还会退到随机名，
  // 而同一个抖音号的 uid_tt 不变（见 store.upsert 与 cookieUid）
