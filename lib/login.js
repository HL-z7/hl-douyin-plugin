 ...
 */
export async function startQrLogin(botId, { accountName = "", userId = "", onQrcode, onStatus, onVerify } = {}) {
  ...
  const tag = `扫码登录 ${s.id}`
  debug(tag, `会话开始：botId=${s.botId} 账号名=${s.accountName || "(登录后读昵称)"} 超时=${timeoutSec}s 远程验证=${config.bool("security.remoteVerify", true) ? "开" : "关"} 无头=${config.bool("spark.headless", true) ? "开" : "关"}`)
...
function armTimer(s, setStatus, ms) {
  ...
  s.timer = setTimeout(async () => {
     ...
     */
    if (s.status !== "verify" && config.bool("security.remoteVerify", true) && s.page && !s.page.isClosed()) {
      ...
      if (hint) {
        log("warn", `扫码登录 ${s.id}：超时兜底时发现页面仍需人工验证（${hint}），改为发远程验证链接`)
...
async function runQrLogin(s, setStatus, onQrcode) {
  ...
  s.context = context
  s.page = page
  debug(tag, "页面已建（未挂请求拦截），UA=", () => browserManager.currentUA())
  ...
  page.on("response", async response => {
    ...
    try {
      if (/passport|sso|qrcode|qrconnect|login/i.test(url)) {
  ...
  while (!s.qrRound && Date.now() < qrDeadline) {
    ...
    if (qrProbes % 10 === 1) debug(tag, `第 ${qrProbes} 轮找二维码：${found ? "DOM 里有" : "还没有"}`)
    emitQr(found)
    if (s.qrRound) break
    await sleep(500)
  }
  if (!s.qrRound) {
    ...
    throw new Error("未能获取二维码，请稍后重试或改用手动登录")
  }
  debug(tag, `二维码已就绪（第 ${qrProbes} 轮探到，第 ${s.qrRound} 张）`)
  ...
  while (!isDone(s)) {
    ...
    if (s.status === "confirmed" && s.confirmedAt && Date.now() - s.confirmedAt > CONFIRM_GRACE_MS) {
      ...
      if (config.bool("security.remoteVerify", true)) {
        markVerify(s, setStatus, "未能识别的验证页，请打开链接自行查看")
        continue
      }
      const shot = await diagnose(s)
      setStatus(
