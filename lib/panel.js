 ...
 * 聊天功能的摘要，状态与设置面板共用。
 *
 * 只报「开着几个会话 / 库里有多少条」，不报库文件路径：这两个面板会被发到群里，
 * 路径等于把服务器目录结构贴出去（web.js 的 chatDbBrief 是同一条判据）。
 */
  ...
  const sections = [
    ...
    },
    {
      ...
      rows: [
        ...
        // 两列布局下这一格只有 ~250px，长会话名会折成两行，所以截短
 ...
 * 顶部的能力概览。
 *
 * 帮助图原来一上来就是二十条指令，看的人得自己从指令名里拼出「这插件能干什么」。
 * 这四格先把四件事说清楚，再往下看指令就知道每条属于哪件事。
 * 顺序按「多数人来干的事」排：续火是本体，聊天是新加的大功能，后两个是登录路上的坎。
 */
const HELP_FEATS = [
 ...
 * 对所有指令都成立的三条约定。
 *
 * 它们不属于任何一个指令分组（写进哪一组都像那一组的特例），
 * 塞进底部提示条又会把那段话撑成一屏，所以单独一条并排的信息带。
 * 每格控制在一行内：三格并排时一格只有 ~250px，折行会把这条带子撑成两倍高。
 */
const HELP_RULES = [
...
export function buildHelpData(botId) {
  // 「（面板内）点账号 → 聊天」不是一条指令，别把它算进右上角那个数字
