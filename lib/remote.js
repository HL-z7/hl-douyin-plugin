...
export async function applyAct(page, act) {
  ...
  if (type === "click" || type === "dblclick") {
     ...
     * 先 move、停一下、再 click。
     *
     * page.mouse.click 内部也会先 move，但它是「同一 tick 里 move→down→up」。
     * 抖音这类组件常把按钮的响应挂在 hover 之后才建立的状态上（React 的
     * onPointerEnter 里改 state，下一帧才把真正的 click 处理器挂上去），
     * 一口气三连发时按钮身上可能还没有处理器。中间那 90ms 就是留给它一帧的时间。
     * 代价是每次点击慢 90ms，对人来说察觉不到。
     */
