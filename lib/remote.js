...
export async function applyAct(page, act) {
  ...
  if (type === "click" || type === "dblclick") {
    const { x, y, vp } = toPixel(page, act.x, act.y)
    await page.mouse.click(x, y, { clickCount: type === "dblclick" ? 2 : 1 })
    trace(`相对 ${Number(act.x).toFixed(3)},${Number(act.y).toFixed(3)}`, `→ 像素 ${x},${y} 视口 ${vp.width}x${vp.height}`)
    return { ok: true }
  }
  ...
  if (type === "drag") {
    ...
    await page.mouse.up()
    const last = toPixel(page, points.at(-1).x, points.at(-1).y)
