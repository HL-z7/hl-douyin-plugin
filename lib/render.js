 ...
 * 出图基准倍率。
 *
 * 清晰度来自这里，而不是 deviceScaleFactor：Yunzai 的渲染器截的是
 * `page.$("#container")` 这个元素（renderers/puppeteer/lib/puppeteer.js:199），
 * 元素截图按 CSS 像素走，调 deviceScaleFactor 对它没有影响——那是整页截图的参数。
 * 而 `transform: scale()` 会体现在元素的 boundingBox 上，于是 900px 的版面
 * ×1.4 就是 1260px 的实际出图。
 *
 * 版面按 900px 设计（与 hl-picture-plugin 同一口径，那套视觉在这个宽度上调好的），
 * 1.4 之后落在 1260px：聊天窗里不会被缩略成看不清，图片体积也还在几百 KB。
 */
...
export async function render(tpl, data = {}) {
  ...
  // 用户倍率夹在 0.6–2 之间：低于 0.6 字已经看不清，高于 2 图片体积暴涨还容易触发发送失败。
  // 它是在 BASE_SCALE 之上的调节量，config 里的 1 表示「按设计稿的清晰度出图」
  const scale = BASE_SCALE * config.num("render.scale", 1, { min: 0.6, max: 2 })
  ...
  try {
    ...
    return await puppeteer.screenshot(`hl-douyin-plugin/${tpl}`, {
      ...
      imgType: "png",
      quality: 100,
      // 本地 file:// 页面没有外部请求，networkidle0 立即满足，不会额外等待
      pageGotoParams: { waitUntil: "networkidle0" },
      version: version(),
      // 整段 style 串交给模板（art-template 的 {{@}} 不转义）。缩放原点必须钉在左上角，
      // 否则放大后的版面会跑出 #container 的 boundingBox，截图边缘被切掉
      sys: { scale: `style="transform:scale(${+scale.toFixed(3)});transform-origin:0 0;"` },
      ...data,
    })
  } catch (error) {
