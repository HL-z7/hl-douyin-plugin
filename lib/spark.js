...
async function searchOnce(page, name, trace = () => {}) {
  ...
  // 结果条数一起报出来：「一条都没有」多半是搜索索引没就绪，「有几条但名字对不上」
  // 才是好友改名或备注不一致 —— 这两种排查方向完全不同
  if (index === false || index === null) {
    const total = await page.$$eval(SEL.searchItem, els => els.length).catch(() => -1)
    trace(`搜「${name}」未命中：结果 ${total < 0 ? "读不到" : `${total} 条`}${cleared ? "" : "（上一轮结果未清空，可能有残留）"}`)
    return null
  }
...
export async function listFriends(botId, accountId) {
  ...
  try {
    ...
    debug("拉取会话列表", `读到 ${names.length} 个昵称`)
