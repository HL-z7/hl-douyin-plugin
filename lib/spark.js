 ...
 *   几组、每组多少成员、最终选了哪一组、有没有下钻。「读到的条数不对」只能靠这份读数定位
 * @returns {Promise<Array<{slot: number, leaves: string[], all: string}>>} evaluate 失败时返回空数组
 */
  ...
  const r = await page
    .evaluate(
      ([sel, attr]) => {
         ...
         * 定位「一条会话」对应的那一层元素。
         *
         * `[class*="conversation"]` 是模糊匹配：一条会话从行到行内的标题块、描述块，每层
         * class 都带 conversation，一个好友被数成十几个节点。要从这一堆节点里挑出「行」那一层。
         *
         * 判据是**按 class token 归组**：同一层的元素由同一个 React 组件渲染，必然共享一个
         * class token（现场是 `conversationConversationItemwrapper`，11 个会话行都带它），而
         * 行内的标题块、描述块各自带别的 token、各成一组。取成员最多的那一组即会话行。
         *
         * 早期实现按 parentElement 归组，在现场这份 DOM 上失效：虚拟列表给每个会话行外面套了
         * 一层定位容器（class 不含 conversation），于是 11 个行的父节点两两不同、每组只有 1 个
        ...
        for (const group of groups)
          if (
            ...
            (group.items.length === picked.items.length && group.depth < picked.depth)
          )
            picked = group

        /*
         * 一组都没有（每个 token 都只命中一个元素）—— 列表里只有一条会话，或者行级 class 已被
         * 混淆得毫无共性。此时取「命中节点中最外层、且真的有文本的那些」，至少能读出一条，不至
         * 于返回空。
         *
         * 要过滤空文本：右侧聊天区容器（现场是 `conversationConversationPanel`）同样命中选择器，
         * 未选会话时它是空的，留着只会变成一条「取不到昵称」的垃圾项。
         */
        if (!picked && hits.length) {
          const items = outermost(hits).filter(el => (el.textContent || "").trim())
          if (items.length) picked = { token: "(无共享 class)", items, depth: 0 }
        }
        ...
        return {
          ...
          diag: {
            hits: hits.length,
            // 只报成员数前几名的组：现场那份 DOM 有上百个 token，全打出来一行几千字
            groups: groups
              .slice()
              .sort((a, b) => b.items.length - a.items.length)
              .slice(0, 5)
              .map(g => `${g.token}×${g.items.length}`),
            total: groups.length,
            picked: picked ? `${picked.token}×${picked.items.length}` : "(一组都没有)",
...
export async function listFriends(botId, accountId, { refresh = false } = {}) {
  ...
  try {
     ...
     *
     * 反过来也踩过：把会话行内部的两个子块当成了两条会话，读出「1 个昵称 + 1 条取不到昵称」。
     * readItems 现在按 class token 归组（同一层由同一个组件渲染、必然共享 token），并把分组
     * 过程写进日志 —— 传 trace 就是为了让这份读数出现在「拉取会话列表」这条路上。
     */
    const items = await readItems(page, SEL.conversation, trace)
    const names = []
    const dropped = []
