...
/**
 * 「整条就是一个时间戳」的判据，供 readItems 数一项里有几条会话用。
 *
 * 与 NOISE_RE 分开而不复用：那个是 `g` 且不带边界（要在粘连文本里逐段剔除），这里要的是
 * 「这个叶子是不是纯时间」—— 带上 `^$` 才能把「19:03」与「今天19:03有空吗」分开，后者是
 * 消息预览，不能计入。「10分钟内在线」也算一个（它同样每条会话最多出现一次）。
 */
const STAMP_RE = /^(刚刚|\d+\s*(分钟|小时|天)前?(内)?(在线)?|在线|\d{1,2}:\d{2}|昨天|前天|星期[一二三四五六日天]|\d{4}[-/]\d{1,2}[-/]\d{1,2}|\d{1,2}[-/]\d{1,2})$/
...
export async function readItems(page, selector, trace = null) {
  const r = await page
    .evaluate(
      ([sel, attr, timeSrc]) => {
        ...
        /*
         * 按父节点归组，返回「成员最多、同样多则更靠外」的那一组。
         *
         * 判据用纯结构、不认 class 名也不认中文文案：会话行是一批同父兄弟，而各层容器每种
         * 只有一个，因此成员最多的那组即会话行。同层的标题行/副标题行总数虽然相同，但分散
         * 在各自会话内部、每组只有一两个，不会被选上。
         *
         * 已排除的两条路径：
        ...
        const first = groupBy(hits)
        let picked = first.picked
        let drilled = ""

        /*
         * 下钻：命中的节点里没有「会话行」那一层时，整张列表被当成了一条。
         *
         * 上面那套判据的前提是「每个会话行自身的 class 里含 conversation」。抖音把 class 混淆
         * 之后随时可能不含 —— 那时命中的只剩几个布局容器，它们互为父子（每组 1 个）或恰好是
         * 两个同父兄弟（左侧列表容器 + 右侧聊天区），于是 picked 落在整张列表上，读出来永远
         * 只有 1 条，且必然是最上面那个好友。现场就是这个样子（日志「2 条会话，读出 1 个
         * 昵称」）。
         *
         * 判据是「这一项里有几个时间戳」，不是字数：一条会话行最多带一个时间（「19:03」
         * 「昨天」这类），而整张列表里有几个好友就有几个。字数阈值试过，定不准 —— 短昵称 +
         * 短预览的三条会话加起来可能还不到一条长预览的字数。
         *
         * 下钻时不再限定 class（行级 class 已证明不可认），任何有文本的元素都算候选，仍按同一
         * 套「同父兄弟最多」的结构判据分组。只有真的分出更多行才采纳。
         */
        const timeRe = new RegExp(timeSrc)
        const stampsIn = el => {
          let n = 0
          for (const node of el.querySelectorAll("*")) {
            if (node.children.length) continue
            if (timeRe.test((node.textContent || "").trim())) n++
          }
          return n
        }
        const worst = (picked?.items || []).reduce(
          (max, el) => {
            const n = stampsIn(el)
            return n > max.n ? { el, n } : max
          },
          { el: null, n: 0 }
        )
        if (worst.el && worst.n >= 2) {
          const inner = [...worst.el.querySelectorAll("*")].filter(el => {
            const text = (el.innerText || el.textContent || "").trim()
            // 超过 2000 字的一定是包着整个列表的容器，不是单条会话
            return text && text.length <= 2000
          })
          const second = groupBy(inner)
          // 只有真的分出更多行才采纳，否则维持原判（宁可少读也不要把行内结构当成会话）
          if ((second.picked?.items.length || 0) > (picked?.items.length || 0)) {
            drilled = `${name(worst.el)}（含 ${worst.n} 个时间戳）→ ${second.picked.items.length} 行`
        ...
        return {
          ...
          diag: {
            hits: hits.length,
            groups: first.groups.map(g => `${name(g.parent)}×${g.items.length}`),
            picked: picked ? `${name(picked.parent)}×${picked.items.length}` : "(一组都没有)",
            drilled,
          },
        }
      },
      [selector, SLOT_ATTR, STAMP_RE.source]
    )
    .catch(() => null)
  ...
  if (trace) {
    const d = r.diag
    trace(
      `readItems(${selector})：命中 ${d.hits} 个节点，分 ${d.groups.length} 组` +
        `${d.groups.length ? `（${d.groups.join(" ")}）` : ""}，选中 ${d.picked}` +
        `${d.drilled ? `，已下钻 ${d.drilled}` : ""} → ${r.out.length} 条`
    )
  }
  return r.out
}
...
export async function listFriends(botId, accountId, { refresh = false } = {}) {
  ...
  try {
     ...
     *
     * 反过来也踩过：行级 class 不含 conversation 时命中的只剩布局容器，整张列表被算成一条，
     * 读出来永远只有最上面那个好友。readItems 现在会在那种形状下下钻一层，且把分组过程写进
     * 日志 —— 传 trace 就是为了让这份读数出现在「拉取会话列表」这条路上。
     */
    const items = await readItems(page, SEL.conversation, trace)
    const names = []
    const dropped = []
