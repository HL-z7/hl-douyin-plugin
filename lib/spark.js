...
import { clickAt, clearText, clickByText, clickHandle, typeText, pressKey, visibleBox } from "./interact.js"
...
/**
 * 等会话列表「条数稳定」的上限与轮询间隔，见 waitForChatList。
 *
 * `waitForSelector(SEL.conversation, {visible:true})` 由最外层容器就能满足，它在任何一行渲染
 * 出来之前就已存在且有尺寸；`waitForNetworkIdle` 也只管网络，不管 React 画完没有。少画一行就
 * 会把 readItems 的分组判据带偏，所以这里再等一轮。
 *
 * 3 秒够用：命中节点数从 0 到稳定通常在 1 秒内，给到 3 秒是慢机器的余量。等不到不抛错。
 */
const LIST_SETTLE_TIMEOUT = 3000
const LIST_SETTLE_POLL = 400
/**
 * 等旧搜索结果收起的上限。
 *
 ...
 * （搜索的输入走 typeText，它的后两级退路不经点击也能把字写进框），点「发消息」的日志也
 * 写着「事件已到 DOM」（到的是浮层），随后等输入框耗满 4 档 31 轮超时——
 * 即「会话能看见，但消息发不出去」。
 */
 ...
 *
 * 等到 selector 只说明「列表区域出现了」，不代表行渲染完 —— 最外层容器满足 visible 的时刻
 * 远早于第一条会话出现。因此额外等一轮「条数稳定」：每 400ms 数一次命中节点，连续两次相同
 * 即认为渲染停了。这一步也不抛错，超时就带着当前读数继续。
 *
...
export async function waitForChatList(page, notify = () => {}, trace = () => {}) {
  ...
  // 等条数稳定：网络空闲了不等于 React 已经把行画完，而下一步就要按结构分组，
  // 少画一行就会把分组判据带偏（成员最多的那一组可能还没成型）
  if (ready) {
    const deadline = Date.now() + LIST_SETTLE_TIMEOUT
    let prev = -1
    let stable = 0
    while (Date.now() < deadline) {
      const n = await page.evaluate(sel => document.querySelectorAll(sel).length, SEL.conversation).catch(() => -1)
      if (n > 0 && n === prev) {
        stable += 1
        if (stable >= 2) break
      } else {
      ...
      prev = n
      await sleep(LIST_SETTLE_POLL)
    }
    trace(`等列表条数稳定：${prev > 0 ? `${prev} 个节点${stable >= 2 ? "（已稳定）" : "（仍在变，继续）"}` : "数不到节点"}`)
  }

  // 浮层探测放在这里：listFriends / sparkOnce / sendCustom 三条路都要先过这一关，
  // 且必须赶在第一次搜索之前——fillSearch 的点击也是真鼠标，同样会被浮层吃掉
 ...
 * @param {(...args: any[]) => void} [trace] 传入时把分组过程写进日志：命中几个节点、分了
 *   几组、每组多少成员与其中几个能读出名字、最终选了哪一组。「读到的条数不对」只能靠这份读数定位
 * @returns {Promise<Array<{slot: number, leaves: string[], all: string}>>} evaluate 失败时返回空数组
 */
export async function readItems(page, selector, trace = null) {
  const r = await page
    .evaluate(
      ([sel, attr, noise]) => {
         ...
         * 定位「一条会话」对应的那一层元素。
         *
         * `[class*="conversation"]` 是模糊匹配：一条会话从行到行内的标题块、描述块、时间标签，
         * 每层 class 都带 conversation，一个好友被数成十几个节点。要从这一堆节点里挑出「行」那层。
         *
         * 第一步**按 class token 归组**：同一层的元素由同一个 React 组件渲染，必然共享一个
         * class token（现场是 `conversationConversationItemwrapper`，13 个会话行都带它），而
         * 行内的标题块、描述块、标签各自带别的 token、各成一组。
         *
         * 第二步挑组，判据有三条，按优先级排：
         *
         * 1. **不能是「一条会话里装了本组多个成员」的那种层**（下面的 inner）。这条是主判据，
         *    也是唯一的结构性判据：会话行在列表里一行一个，而行内的标签、按钮往往一行好几个。
         *    现场就栽在这里——日志「前 5：ConversationItemTagNextToTitleleft×26
         *    conversationConversationItemwrapper×13 …），选中 ConversationItemTagNextToTitleleft×26
         *    → 26 条」「26 条会话，读出 10 个昵称，丢弃 16 条（刚刚、刚刚、…、22:38、20:09）」：
         *    每个会话行内挂着 2 个 `TagNextToTitle`（一个装标签文字、一个装时间），13 行共 26 个，
         *    按成员数比就赢过了行本身，读出的 10 个「昵称」其实是标签文字，正是用户看到的
         *    「拉取的好像都不是好友」。
         * 2. **去噪后互不相同的文本更多**。会话行各是一个人、值必然分散；行内标签是「抖友」
         *    「置顶」这类固定字样反复出现，去重后只剩一两个。这条挡住的是「标签也一行一个、
         *    躲过了第 1 条」的情形。
         * 3. 并列时取更靠外的一层：wrapper 与它内部的 rowArea、titleWrapper 数量与文本都相同，
         *    要的是最外面那个 wrapper（它才同时含昵称、时间与预览）。
         *
         * 刻意不用「成员最多」当判据：那正是上面挑中标签层的原因。也不用「有多少成员能读出
         * 昵称」——它对现场这份 DOM 成立（26 个标签里 16 个只有时间）纯属巧合，标签文字一变成
         * 条条都有，同样的比法又会挑中标签层。
         *
         * 早期实现按 parentElement 归组，在现场这份 DOM 上失效：虚拟列表给每个会话行外面套了
         * 一层定位容器（class 不含 conversation），于是各行的父节点两两不同、每组只有 1 个
         * 成员，而「某一行内部的 2 个子块」反倒成了成员最多的组（日志「分 104 组…选中
         * div.conversationConversationItemwrapper×2 → 2 条」，读出来的两条是同一个好友的标题块
         * 与描述块，后者取不到昵称被丢弃，最终只剩 1 条）。
         *
         * 当年排除 class 归组的理由是「抖音会给选中行挂 active 之类的额外 class」——那只影响
         * 完整 class 串比对，按单个 token 归组不受影响：多出来的 token 只是让该元素多进一个组，
         * 它仍然在共享的那个组里。
         *
         * 另两条排除掉的路径：
         ...
         * - 认「同父兄弟最多」：即上面那个失效的实现
         */
        const noiseRe = new RegExp(noise, "g")
        /**
         * 一条会话的文本上限。超过的一定是包着多行的容器（虚拟列表的分块容器动辄上千字），
         * 不是行本身。返回空串即「这一层不可能是一条会话」
         */
        const ROW_CAP = 400
        const rowText = el => {
          const text = (el.textContent || "").replace(/\s+/g, " ").trim()
          return text && text.length <= ROW_CAP ? text : ""
        }
        /** 去掉时间与在线状态后剩下的部分，判据与 itemName 同源。空串 = 这一层只有时间 */
        const bareText = el => rowText(el).replace(noiseRe, "").trim()

        const byToken = new Map()
        for (const el of hits) {
          for (const token of String(el.getAttribute("class") || "").trim().split(/\s+/)) {
            if (!token) continue
            if (!byToken.has(token)) byToken.set(token, [])
            byToken.get(token).push(el)
          }
        }
         ...
         * 组内互相嵌套的要剔掉，只保留最外层那些。
         *
         * 抖音有些 token 同时挂在外层与内层（例如列表容器和它内部的行都带某个前缀 token），
         * 不剔就会把「1 个容器 + N 个行」算成 N+1 个成员，选中它之后每个行被读两次。
         */
        const outermost = list => list.filter(el => !list.some(o => o !== el && o.contains(el)))

        const groups = []
        for (const [token, list] of byToken) {
          const items = outermost(list)
          // 单成员的组留着也无用：会话列表至少两条才需要区分，一条时下面的 fallback 兜住
          if (items.length < 2) continue
          const texts = items.map(bareText).filter(Boolean)
          // 一个能读出名字的成员都没有 —— 这一层装的全是时间，不可能是会话行。
          // 现场那个只有一条会话的号就栽在这里：唯一成组的 token 是标签层（2 个成员，
          // 内容是「(空)」与「21分钟前」），不排掉它就会选中它并读出 0 个昵称
          if (!texts.length) continue
          // 深度用于并列时取更靠外的那一层：wrapper 与它内部的 rowArea 数量相同，要取 wrapper
          let depth = 0
          for (let p = items[0].parentElement; p; p = p.parentElement) depth++
          groups.push({ token, items, depth, uniq: new Set(texts).size, inner: false })
        }

        /*
         * 标出「一条会话里装了本组多个成员」的层。
         *
         * 判据：本组某个成员向上，遇到的第一个「也是别的组的成员」的祖先里，装着本组 2 个以上
         * 的成员。会话行在列表里一行一个，行内的标签、按钮才会一行好几个。
         *
         * 只把「别的组的成员」当候选容器，而不是任何祖先：虚拟列表的定位容器与整张列表的外壳
         * 都是独一份的，不成组，天然被排除在外 —— 否则「整张列表装着 13 个行」会让行层也被判成
         * 行内零件。
         */
        const memberOf = new Map()
        for (const g of groups)
          for (const el of g.items) {
            if (!memberOf.has(el)) memberOf.set(el, new Set())
            memberOf.get(el).add(g.token)
          }
        for (const g of groups) {
          for (const el of g.items) {
            for (let p = el.parentElement; p && !g.inner; p = p.parentElement) {
              const tokens = memberOf.get(p)
              if (!tokens || (tokens.size === 1 && tokens.has(g.token))) continue
              let n = 0
              for (const o of g.items) if (p.contains(o) && ++n > 1) break
              g.inner = n > 1
              break
            }
            if (g.inner) break
        ...
        for (const group of groups)
          if (
            !picked ||
            // 一行装多个的层最先被排掉：会话行在列表里一行一个，标签、按钮才会一行好几个
            (picked.inner && !group.inner) ||
            (picked.inner === group.inner &&
              // 去噪后互不相同的文本更多者胜：每条会话是不同的人，行内标签则是固定字样反复出现
              (group.uniq > picked.uniq ||
                // 并列时取更靠外的那一层：会话行一定在它自己的内部结构之上
                (group.uniq === picked.uniq && group.depth < picked.depth)))
          )
            picked = group

        /*
         * 一组都没有 —— 每个 token 都只命中一个元素（列表里只有一条会话），或者成组的那些层
         * 全是时间（上面 texts 那条把它们排掉了）。此时退到「命中节点里能读出名字、且最靠外的
         * 那些」，至少读出一条，不至于返回空。
         *
         * 先按 bareText 过滤再取最外层，两步不能颠倒：包着整张列表的容器同样命中选择器，它文本
         * 上千字、被 ROW_CAP 挡掉，先过滤才能让真正的行浮到最外层。
         * 未选会话时右侧空的聊天区容器（现场是 `conversationConversationPanel`）也在此被排掉，
         * 否则会多出一条「取不到昵称」的垃圾项。
         */
        if (!picked && hits.length) {
          const items = outermost(hits.filter(bareText))
          if (items.length)
            picked = { token: "(无共享 class)", items, depth: 0, inner: false, uniq: items.length }
        }
        ...
        return {
          ...
          diag: {
            hits: hits.length,
            // 只报前几名的组：现场那份 DOM 有上百个 token，全打出来一行几千字。
            // 每组报「成员数 / 去噪后互不相同的文本数 / 是否一行装多个」——挑错层时正是这几个
            // 数不成比例（标签层成员多而 uniq 极小，且 inner 为真）
            groups: groups
              .slice()
              .sort((a, b) => Number(a.inner) - Number(b.inner) || b.uniq - a.uniq || a.depth - b.depth)
              .slice(0, 5)
              .map(g => `${g.token}×${g.items.length}(异${g.uniq}${g.inner ? " 行内" : ""})`),
            total: groups.length,
            picked: picked
              ? `${picked.token}×${picked.items.length}(异${picked.uniq}${picked.inner ? " 行内" : ""})`
              : "(一组都没有)",
            sample: picked?.items[0] ? name(picked.items[0]) : "",
          },
        }
      },
      [selector, SLOT_ATTR, NOISE_RE.source]
    )
    .catch(() => null)
  ...
  if (trace) {
    const d = r.diag
    trace(
      `readItems(${selector})：命中 ${d.hits} 个节点，${d.total} 个 class 分组` +
        `${d.groups.length ? `（前 ${d.groups.length}：${d.groups.join(" ")}）` : ""}` +
        `，选中 ${d.picked}${d.sample ? ` 形如 ${d.sample}` : ""} → ${r.out.length} 条`
    )
  }
  return r.out
}
...
async function findInChatList(page, names, trace = () => {}) {
  ...
  const items = await readItems(page, SEL.conversation, trace)
  if (!items.length) {
 ...
 * 清空并输入搜索词。
 *
 * 不直接改 `el.value`：搜索框是受控组件，赋值不触发 React 的 onChange，抖音那边的搜索状态
 * 不会更新。
 *
 * 输入走 lib/interact.js 的 typeText，而不是 `elementHandle.type()`。后者只有真键盘一条路
 * （`Input.dispatchKeyEvent`），这个页面上它进不了 DOM —— 现场日志连着好几轮
 * 「⚠ 搜索框里是「」，与要搜的「ciallo」不一致」后跟「结果 0 条」，字一个都没落进框里，
 * 于是「搜不到好友」的表现与「抖音确实没这个人」完全一样。typeText 在真键盘之后还有
 * insertText 与合成输入两级退路，且每级都回读确认。
 *
 * 清空也要走同一条路：clearText 先按 Backspace，回读仍非空再直接改值 —— 光按 Backspace 在
 * keydown 被拦的页面上只删掉一个字符（实测读回 `ciall`），残留会污染下一次搜索。
 *
...
async function fillSearch(page, value) {
  ...
  try {
    // 先点一下：焦点与 window.__dyField 都由这一步交给 interact 层，后面几级退路要用它们
    await clickHandle(page, input, { scope: "搜索框" })
    await clearText(page, input, { scope: "搜索框" })
    if (value) await typeText(page, value, { scope: "搜索框", delay: 40 })
    return await input.evaluate(el => el.value ?? "").catch(() => "")
  } finally {
...
/**
 * 清空搜索框并等结果面板收起，让会话列表回到可见状态。
 *
 * 给 lib/chat.js 用：那边读会话列表之前必须先清搜索框 —— openPeer 走 searchConversation 会把
 * 昵称留在框里，搜索面板一直开着并盖住会话列表，此后每次「刷新会话」都只读到搜索结果那几条。
 * spark 自己的列表兜底 findInChatList 早就这么做了，这里把同一段抽出来共用，不在 chat.js 里
 * 复制第二份（fillSearch 是模块私有的，chat.js 拿不到）。
 *
 * 不抛错：清不掉也只是列表读数偏少，不该让整次刷新失败。
 *
 * @returns {Promise<void>}
 */
export async function clearSearch(page) {
  await fillSearch(page, "").catch(() => "")
  // 等结果面板真的收起。等不到也往下走：残留项无害，匹配是按名字挑的
  await page
    .waitForFunction(sel => !document.querySelector(sel), { timeout: SEARCH_CLEAR_TIMEOUT }, SEL.searchItem)
    .catch(() => {})
  await sleep(SEARCH_RESET_DELAY)
...
export async function listFriends(botId, accountId, { refresh = false } = {}) {
  ...
  try {
    ...
    const trace = (...a) => debug("拉取会话列表", ...a)
    await waitForChatList(page, () => {}, trace)
     ...
     * `10分钟内在线` 是同一条会话的五个嵌套层级。
     *
     * 反过来也踩过两次，都是「挑中了行以外的那一层」：
     * - 按同父兄弟归组 → 选中一行内部的两个子块，读出「1 个昵称 + 1 条取不到昵称」
     * - 按 class token 归组、取成员最多的组 → 选中行内的标签层（每行 2 个、13 行共 26 个，
     *   比行本身还多），读出的「昵称」其实是标签文字与时间
     * readItems 现在按 class token 归组，再排掉「一行装多个」的层、比「去噪后互不相同的文本
     * 数」挑组，并把这份读数写进日志 —— 传 trace 就是为了让它出现在「拉取会话列表」这条路上。
     */
    const items = await readItems(page, SEL.conversation, trace)
    const names = []
    const dropped = []
    for (const item of items) {
      ...
      if (!name) {
        dropped.push(oneLine(item.all, 24) || "(空)")
        continue
      }
      if (names.includes(name)) {
        dropped.push(`${name}(重复)`)
        continue
      }
      names.push(name)
    }

    // 读到的条数与最终昵称数不一致时把丢掉的那些也报出来：差值本身说明不了原因，
    // 而「取不到名字」与「同名重复」两种成因的排查方向不同
    debug(
      "拉取会话列表",
      `${items.length} 条会话，读出 ${names.length} 个昵称` +
        (dropped.length ? `，丢弃 ${dropped.length} 条（${dropped.join("、")}）` : "")
    )
