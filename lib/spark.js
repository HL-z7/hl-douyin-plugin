...
/*
 * 下面几个常量与函数导出给 lib/chat.js（聊天界面）用。
 *
 * 刻意让聊天界面复用这里的东西、而不是自己再抓一遍 DOM：抖音的 class 名每次发版都可能
 * 变，选择器分档、浮层处理、「兄弟最多的一组才是会话行」这些判据全是被现场打出来的。
 * 分散到两个文件里，抖音一改版就变成「修了一处漏一处」。
 */
 ...
 * 它不报错也不挡渲染，只把点击全吃掉，于是表现成最难查的一种失败：会话搜得到
 * （搜索走 elementHandle.type()，DOM focus 绕过浮层），点「发消息」的日志还写着
 * 「事件已到 DOM」（到的是浮层），然后等输入框等满 4 档 31 轮超时 ——
 * 也就是「会话能看见，但是发不出去」。
 */
const OVERLAY_ATTR = "data-dy-overlay"

/** 最多连清几层。抖音偶尔叠两个弹窗，但清不掉时不能在这儿死循环 */
const OVERLAY_ROUNDS = 3

/** 点完等浮层收起的时间，够抖音那点淡出动画走完 */
const OVERLAY_SETTLE = 400

/**
 * 关浮层的按钮文本，从先到后试。
 *
 * 只点「不做事」的那个：「保存」「确定」「同意」「允许」都会改账号状态或权限，
 * 插件没被授权替用户做这种决定；何况每次续火都是全新上下文，答不答下次都会再问。
 */
const OVERLAY_DISMISS = [
  /^(暂不保存|不保存|暂不|取消|以后再说|下次再说|不用了|不再提醒|不再提示)$/,
  /^(知道了|我知道了|跳过|关闭)$/,
  // 按钮文本带了没预料到的后缀时（「不保存密码」这种）用这一档。范围限死在弹窗内部，
  // clickByText 又会往里收到「文本含这几个字的最小元素」，落点还是按钮本身
  /暂不|不保存|不再提醒|不再提示|以后再说|下次再说/,
]

/**
 * 放开范围、在整页里找的那一档，用词比上面严得多。
 *
 * 遮罩和按钮有时是兄弟节点，此时按钮不在弹窗根节点里面，只能整页找。而 clickByText
 * 兜底的合成事件是直接打在元素上、绕过遮罩的 —— 万一在整页里匹配歪了，真会点到东西。
 * 所以这一档只留「除了这种弹窗几乎不会出现」的字眼，不含「取消」「关闭」这类到处都有的词。
 */
const OVERLAY_HARD_DISMISS = [/^(暂不保存|不保存|以后再说|下次再说|不再提醒|不再提示)$/]

/** 关闭图标没有文本，只能按 class/aria 找。范围限定在浮层内部，点不到页面上别的东西 */
const OVERLAY_CLOSE_SEL = '[class*="close"], [class*="Close"], [aria-label*="关闭"], [aria-label*="close"]'

/**
 * 长得像验证/风控面板的浮层一律不碰。
 *
 * 把它点掉等于把「需要人工验证」变成「莫名发不出去」——验证本来有 lib/remote.js
 * 那条人工接管的路，而这里一按「取消」就把它关了，用户只剩一句「发不出去」。
 * 宁可这次失败得明明白白。
 *
 * 分成 id/class 和正文两把尺子，因为两边的误判代价不一样：
 *   - id/class 是框架给的语义词，稳且几乎不会撞（现场那个验证面板的 id 是
 *     `uc-second-verify`，落在 /verif/ 上），可以放宽。
 *   - 正文只留验证面板独有的整词。像「风险」「安全」这种，「是否保存登录信息」
 *     那类弹窗的正文里也常写（「请勿在公共设备上保存，存在被盗风险」），
 *     一杆子打宽了就会把本该点掉的浮层放过去 —— 而那正是这次要修的 bug。
 */
 ...
 *
 * 值是 `{at, why}` 而不是单个时间戳：加进聊天界面之后，一个账号被占住的原因可能是
 * 「正在续火」也可能是「聊天窗开着」，而这两种的下一步完全不同（前者等一会儿就好，
 * 后者要提示用户先关掉聊天）。错误信息里必须带上是谁占的，否则用户只会看到
 * 「该账号正在执行任务」然后无从下手。
 */
...
/**
 * 抢账号锁。抢不到就抛，错误信息里带上当前占用者。
 *
 * 导出给 lib/chat.js：聊天界面要长期持有一个抖音页面，而同一份 Cookie 同时活在两个
 * BrowserContext 里，抖音那边会看到两个并发登录会话——那是掉线和触发验证的常见起因。
...
export async function waitForChatList(page, notify = () => {}, trace = () => {}) {
  ...
  // 而且要赶在第一次搜索之前 —— fillSearch 那下三连击也是真鼠标，一样会被吃掉
 ...
 * 探一遍盖住页面的浮层，能点掉就点掉。
 *
 * 判据是「这几个点底下坐着谁」而不是遍历全页找 mask：真能吃掉点击的只有命中测试里
 * 最上面那一层，问 `elementFromPoint` 一句就够，也不会因为抖音换个 class 名就漏掉。
 *
 * 找到落点之后往上爬到 body 的直接子节点，拿整棵子树当 root 去找关闭按钮 ——
 * 现场那个浮层是 `div.trust-login-dialog-mask < div#trust-logout-dialog < body`，
 * 遮罩和「暂不保存」是兄弟关系，只拿遮罩当 root 是找不到按钮的。
 *
 * 循环是为了抖音偶尔叠两层（关掉「保存登录信息」后面还压着个引导），
 * 但只循环 OVERLAY_ROUNDS 次：关不掉时得让它带着告警往下走，而不是在这儿转。
 *
 * @param {object} [opts]
 * @param {boolean} [opts.esc] 允不允许按 Escape 兜底。搜索结果面板开着时不能按 ——
 *   Escape 会把面板一起收掉，手里那个结果项 handle 就废了
...
export async function dismissOverlay(page, trace = () => {}, opts = {}) {
  ...
  for (let round = 1; round <= OVERLAY_ROUNDS; round++) {
    ...
    if (!how) {
      /*
       * 关不掉时把浮层的整段文本报出来（不是 40 字的摘要）。
       *
       * 下一步必然失败，而失败信息是「输入框没出现」——跟真正的原因隔着十几行日志。
       * 这里的文本里带着按钮文字，词表漏了哪个词一眼能看出来，补一条正则就好。
       */
 ...
 * 找出正盖着聊天页的那一层。
 *
 * 判据不是「谁的 class 里有 mask」——抖音的混淆 class 发一次版就换。这里问的是
 * **点下去会打到谁**：在搜索框中心与视口几个位置上各做一次 `elementFromPoint`
 * （它天然跳过 `pointer-events:none` 的层，所以能被它返回就证明这层真会吃掉点击）。
 *
 * 拿到落点之后往上爬，爬到「再往上一层就把搜索框也装进去了」为止 —— 也就是
 * **和搜索框那条祖先链岔开的最外那个节点**，它就是这个弹窗的根。现场那个浮层是
 * `div.trust-login-dialog-mask < div#trust-logout-dialog < body`，于是爬出
 * `div#trust-logout-dialog`：遮罩和「暂不保存」是兄弟，只拿遮罩当 root 找不到按钮，
 * 得要它俩的共同父节点。这么爬也顺带管住了弹窗挂在抖音自己页面根节点里面的情形
 * （那时爬出来的是页面根节点里的弹窗子树，不是整个页面）。
 *
 * 认定是浮层还要再满足两条，少一条都会误伤：
 *   1. 那棵子树里没有搜索结果项 —— 结果面板也是 body 底下的独立一层（Semi 的
 *      portal），可它正是我们要点的东西，关掉等于自断退路。
 *   2. 落点到根之间有个 `position:fixed` 且吃点击的块铺满至少六成视口 —— 遮罩必然
 *      是这样，否则它挡不住「点哪儿都没反应」；而下拉菜单、气泡提示只占一小块，
 *      抖音自己的聊天区虽然也占大半屏，却不是 fixed。
 *
  ...
  return page
    .evaluate(
      (searchSel, itemSel, attr) => {
        ...
        // 搜索框是「页面自己」的锚。没有它就无从判断哪层是弹窗、哪层是抖音的界面
        ...
        /*
         * 落点到这一层之间，「铺满视口的固定定位块」最大占了几成。
         *
         * 两个限定都是必需的：
         *
         * `position: fixed` —— 光看面积会把抖音自己的界面认成遮罩：探测点落在聊天区时，
         * 聊天区本身就占七成视口，而它和侧栏里的搜索框是兄弟子树，前面那些判据一条都拦不住。
         * 而要「盖住整页、连滚动都跟着的遮罩」，实现上只能是 fixed。absolute 也做得到，
         * 但普通版面里 absolute 铺满一大片太常见了，放进来等于把误判又请回来 ——
         * 漏判的代价只是回到今天的行为（等输入框超时），误判却会去点抖音正经界面上的东西。
         *
         * 不能只量 layer 自己：弹窗的外层容器常是 0×0 的静态节点，真正铺满的是里面那个
         * fixed 遮罩（现场就是 `div#trust-logout-dialog` 套 `div.trust-login-dialog-mask`）。
         * 所以从落点往上一路取最大值。
         */
        const coverage = (from, stop) => {
          ...
          for (let node = from; node; node = node === stop ? null : node.parentElement) {
            ...
            // 组件库的 portal 容器就是「铺满整屏 + pointer-events:none」
        ...
        // 搜索框中心排第一：它是三条路都要先动的元素。后面几个点管住「弹窗只压住半边」
        const pts = [
        ...
        for (const [px, py] of pts) {
          ...
          // 落点在搜索框那条祖先链上：这里露着的就是抖音自己的界面，不是浮层
...
/**
 * 试着关掉一个浮层，四档依次退，每档点完都回头确认它真的没了。
 *
 * 「点完确认」这一步不能省：这类弹窗的按钮常带自己的动画层，clickByText 报 ok
 * 只说明事件送到了，不代表弹窗收了。少了确认，下面就会把「点了但没关」当成成功，
 * 又绕回「会话能看见、发不出去」那种不报错的失败。
 *
 * @returns {Promise<string>} 走的是哪一档；空串表示四档都没关掉
 */
async function closeOverlay(page, root, trace = () => {}, opts = {}) {
  // ① 弹窗内部的「暂不保存」这类按钮。绝大多数情况在这一档就结束了
  ...
  // ② 右上角那个叉。它没有文本，只能按 class/aria 找，范围限死在弹窗内部
  ...
  // ③ Escape。点不到东西的弹窗有时认这个键，而且它不会误伤页面上别的元素。
  //    但它会顺手收掉搜索结果面板，所以进会话前那次（手里攥着结果项 handle）跳过这档
  ...
  // ④ 放开范围在整页找。遮罩与按钮是兄弟节点、按钮不在弹窗根里时只剩这条路，
  //    所以词表比 ① 严得多（见 OVERLAY_HARD_DISMISS）
 ...
 * 刚才标记的那层浮层还挡不挡路。
 *
 * 判据只有一条：**那几个探测点上，点击还会不会落进这棵子树**。
 *
 * 不量它自己的 rect 与样式（第一版就是那么写的，被探针抓了出来）：浮层的根节点
 * 常是个静态 div，里面两个 `position:fixed` 的孩子（现场就是
 * `div#trust-logout-dialog` 套遮罩 + 对话框），静态父节点撑不起 fixed 孩子的高度，
 * 于是它的 `height` 恒为 0 —— 按尺寸判会每轮都读成「已关掉」，白报三次成功，
 * 弹窗一动没动。而 `elementFromPoint` 天然跳过 display:none / visibility:hidden /
 * 已卸载的节点，「还挡不挡路」这个问题它一句就答完了。
 */
...
/**
 * 从一条会话项里拆出「消息预览」与「时间戳」，给聊天界面的会话列表用。
 *
 * 和 itemName 放在一起而不是丢给 lib/chat.js 自己写：判据要用同一份 NOISE_RE。
 * 抖音把昵称、时间、在线状态、最近一条消息的预览全挤在同一条会话项里，没有任何
 * 独立的预览元素可认，所以只能靠「排除法」——去掉昵称、去掉纯噪音，剩下里最长的
 * 那条就是预览（预览是一句话，时间和在线状态都是短串）。
 *
 * @param {{leaves?: string[]}} item readItems 返回的一项
 * @param {string} name itemName(item) 的结果，要从预览候选里排掉
 * @returns {{preview: string, stamp: string}}
 */
export function itemPreview(item, name) {
  ...
  // 时间戳取第一条「整条都是噪音、且带数字」的叶子：「10分钟内在线」也是纯噪音但
  // 不该当时间用，所以还要求它不含「在线」
...
export async function openChat(page, item, trace = () => {}) {
   ...
   * 进会话之前再探一次浮层。
   *
   * 不能只靠 waitForChatList 那一轮：「是否保存登录信息」是页面就绪之后好几秒才弹的，
   * 而这一步在多目标续火里会被反复走到（每个好友一次），中途弹出来的浮层只有在这里能拦住。
   * 现场那次就栽在这：日志写着「点「发消息」：事件已到 DOM」，到的其实是
   * `div.trust-login-dialog-mask`，接着 waitForEditor 等满 4 档 31 轮。
   * 没有浮层时它只是一次 evaluate，多目标下的开销可以忽略。
   *
   * esc: false —— 这会儿搜索结果面板正开着，手里的 item 就是面板里的一项，
   * 按 Escape 会把面板收掉、handle 跟着废掉，那是自己把路堵死。
   */
...
/**
 * 会话已经打开的情况下，把一条消息打进输入框并发出去。
 *
 * 从 sendMessage 里拆出来，因为聊天界面（lib/chat.js）的会话是常开的：用户在网页上
 * 点了好友，抖音那边的会话就一直停在那里，再走一遍 openChat 等于把已经打开的会话
 * 重新点一次——多一次点击、多一次浮层探测，还可能因为搜索面板已经收起而点不到东西。
 */
...
export async function listFriends(botId, accountId, { refresh = false } = {}) {
  ...
  try {
    ...
    // 空结果不写缓存：拉到 0 个多半是页面没渲染完或 ck 半失效，缓存下来等于把
    // 一次偶发失败固化成几小时的「这个号没有好友」
