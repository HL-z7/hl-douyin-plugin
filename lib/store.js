 ...
 *
 * `uid` 补空串：这个字段是后来加的（upsert 的判重键，见 cookieUid），老账号文件里没有。
 * 不补的话它是 undefined，而 `find(a => a.uid === uid)` 在 uid 也取不到时会拿
 * `undefined === ""` 比较 —— 虽然当前调用方保证了 uid 非空才查，但留个 undefined 在结构里
 * 迟早会被别处的 `!acc.uid` 之类误判。老账号的真实 uid 在下一次 upsert 带 cookie 时补上。
 */
function migrateAccount(acc) {
  if (!acc || typeof acc !== "object") return acc
  if (typeof acc.uid !== "string") acc.uid = ""
...
/**
 * 从一份 cookie 里取抖音账号的稳定身份，供 upsert 判重。
 *
 * `uid_tt` 是抖音给账号的固定标识：改昵称、重新扫码登录都不变，因此它是判「这两次登录是不是
 * 同一个号」唯一可靠的依据。退路 `uid_tt_ss` 是同值的 ss 变体（另一个域下）。
 *
 * 不用 sessionid：那是会话凭证，每次登录都换新值，拿它判重等于永不命中。
 *
 * @param {Array<{name: string, value: string}>|null} cookies
 * @returns {string} 取不到时返回空串，调用方据此退回按名字判重
 */
function cookieUid(cookies) {
  if (!Array.isArray(cookies)) return ""
  const hit = cookies.find(c => c.name === "uid_tt") || cookies.find(c => c.name === "uid_tt_ss")
...
class AccountStore {
   ...
   *
   * 定位规则三级，依次尝试：
   * 1. `input.id` —— 面板与锅巴改已有账号时都带 id，最准
   * 2. cookie 里的 `uid_tt` —— 同一个抖音号重新登录时命中它，即使昵称已经变了
   * 3. `name` —— 手动粘贴 Cookie 且 uid_tt 缺失时的退路
   *
   * 第 2 级是现场加的：原本只有 1、3 两级，而扫码登录不带 id、账号名来自 readNickname
   * （读不到时退到 `抖音账号${随机四位}`），于是同一个号第二次登录只要昵称读数不同就插一条
   * 新记录 —— 用户看到账号列表里同一个号出现两次。
   *
   * uid 命中而名字不同时以新名字为准：用户在抖音改了昵称，列表应当跟着更新。
   * 只有出现在 input 里的字段被覆盖，未出现的保持原值。
  ...
  upsert(botId, input) {
    ...
    // cookie 提前解析：判重要用其中的 uid_tt，落盘也用同一份，不重复解析
    let cookies = null
    if (input.cookie) {
      cookies = Array.isArray(input.cookie) ? input.cookie.map(normalizeCookie) : parseCookieInput(input.cookie)
      assertLoginCookie(cookies)
    }
    const uid = cookieUid(cookies)

    let acc = null
    if (input.id) acc = data.accounts.find(a => a.id === input.id)
    if (!acc && uid) acc = data.accounts.find(a => a.uid === uid)
    if (!acc) acc = data.accounts.find(a => a.name === name)
    const now = Date.now()
    ...
    acc.name = name
    // 先记下旧 uid：下面判「是否换了号」要用它，而紧接着就会被新值覆盖
    const prevUid = String(acc.uid || "")
    // uid 只在取到时写：手动粘贴的残缺 Cookie 不该把已记下的 uid 抹掉
    if (uid) acc.uid = uid
    ...
    // cookies 在函数开头已解析并校验过（判重要用其中的 uid_tt），这里只负责落盘
    if (cookies) {
       ...
       * 会话列表缓存作废，但只在换了号的时候。
       *
       * uid 与上次相同说明这次登录的还是同一个抖音号，好友名单不会因为换了一份 sessionid 就
       * 变化，清掉只是让下一次拉取白等一轮（那一步要真开浏览器、翻会话列表）。取不到 uid 或
       * uid 变了则一律清掉 —— 展示上一个号的好友名单会让用户点错人。
       */
      if (!uid || prevUid !== uid) delete acc.friendsCache
    }
