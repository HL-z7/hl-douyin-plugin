...
class AccountStore {
  ...
  remove(botId, accountId) {
     ...
     * 聊天记录跟着账号一起走。
     *
     * 放在这里而不是三个调用点（apps/login.js、guoba.support.js、lib/web.js）各写一遍：
     * 「账号没了它的聊天记录也不该留」是一条数据生命周期约束，属于删除动作本身。
     * 分散到调用点就是等着第四个入口忘掉——而忘掉的后果是 Cookie 都删干净了，
     * 私聊内容还躺在 chat.db 里。
     *
     * 只删库不管聊天会话：关会话要 import lib/chat.js，而它 import 了本模块，
     * 那是一个循环依赖。会话由调用方负责先关（见 lib/web.js 的删账号路由）。
     */
