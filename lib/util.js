 ...
 * 而需要保护的从来只是主机 —— 用公网 IP 直连的用户，群里贴一次地址就等于把机器暴露给
 * 所有群成员（端口扫描、暴力破解都从这一步开始）。所以主机打码、协议与路径留着。
 *
 * 主机尾部留 3 个字符：本人能认出「是我那台」，别人凑不出完整地址。
 * 端口单独留着无害 —— 没有主机的端口号不指向任何东西。
 *
...
export function maskUrl(value) {
  ...
  try {
    ...
    return `${u.protocol}//${maskSecret(u.hostname, 0, 3)}${u.port ? `:${u.port}` : ""}${u.pathname}${u.search}${u.hash}`
  } catch {
