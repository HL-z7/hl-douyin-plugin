 ...
 * 而 console.log 不依赖 this，仍能正常输出。读法必须是 `globalThis.logger`：Yunzai 里它
 * 就是那个全局 logger，而裸写 `logger` 在未注入的进程里直接 ReferenceError
 * （ESM 严格模式下未声明标识符不返回 undefined），那样「退到 console.log」根本走不到。
 *
...
export function log(level, ...args) {
  ...
  const fn = target?.[level] || console.log
  fn.call(target, LOG_TAG, ...args)
}
...
/**
 * 深拷贝纯数据节点：对象、数组、原始值。配置树只含这三类，Date / Map 等不做特例。
 * deepMerge 用它把两边的引用都断开（理由见下）。
 *
 * @param {*} value
 * @returns {*} 结构相同、引用全新的副本
 */
function cloneNode(value) {
  if (Array.isArray(value)) return value.map(cloneNode)
  if (value && typeof value === "object") {
    const out = {}
    for (const [key, item] of Object.entries(value)) out[key] = cloneNode(item)
    return out
  }
  return value
}
 ...
 *
 * 两边都深拷贝，**合并结果不与 base / override 共享任何子对象**。这条以前只是句注释、
 * 实现上是逐层浅拷贝，于是留下一个很安静的坑：用户配置里没写到的段（比如只写了 spark
 * 的 config.yaml 里的 `chat`），合并结果里那一项就是 DEFAULT_CONFIG 里那个对象本身，
 * 而 config.set / setPath 是原地改内存树的 —— 一次 `#抖音设置 聊天 关` 就把
 * DEFAULT_CONFIG.chat.enable 改成了 false，「默认值」从此不再是默认值。
 * 表现是：删掉配置项、换一份最小配置、热重载读回默认值，都回不到代码里写的那个值；
 * 锅巴表单与 README 以 DEFAULT_CONFIG 为准，也会跟着漂。
 *
 * @param {object|Array} base 不被修改（深拷贝后作为起点）
 * @param {object} override 顶层为空/undefined 时等价于拷贝 base
 * @returns {object|Array} 新对象
 */
export function deepMerge(base, override) {
  const out = cloneNode(base)
